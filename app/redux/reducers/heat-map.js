...
export default function heatMapReducer(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case actionTypes.FETCH_DATA_SUCCESS:
            return immutableState.setIn(['heatMaps', action.payload.id, 'onState'], onState.drawn).setIn(['heatMaps', action.payload.id, 'config', 'series', 0, 'data'], action.payload.mapData.data).toJS();
